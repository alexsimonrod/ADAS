...
class DeepdriveDatasetWriter(object):
    ...
    def feature_dict_description(type='feature_dict'):
        ...
        if type == 'reading_shape':
            ...
            obj['image/object/class/text'] = tf.VarLenFeature(tf.string)
            #obj['image/object/class/label/name'] = tf.VarLenFeature(tf.string)
        return obj

    def __init__(self):
        self.input_path = '/scratch/tfg-adas/datasets/deepdrive'
    ...
    def get_image_label_folder(self, fold_type=None, version=None):
        ...
        valid_folder_structure_new_format = (len(DeepdriveDatasetDownload.filter_files(full_labels_path)) == 2 and \
                                             len(DeepdriveDatasetDownload.filter_files(full_images_path)) > 0)
        print(valid_folder_structure_old_format)
        print(valid_folder_structure_new_format)
        ...
        if valid_folder_structure_new_format:
            full_labels_path = "/scratch/tfg-adas/datasets/deepdrive/labels/bdd100k/labels"
        ...
        for frame in annotations_for_picture_id['frames']:
            for obj in frame['objects']:
                if 'box2d' not in obj:
                    continue
                boxid.append(obj['id'])
                xmin.append(obj['box2d']['x1']/float(image_width))
                xmax.append(obj['box2d']['x2']/float(image_width))
                ymin.append(obj['box2d']['y1']/float(image_height))
                ymax.append(obj['box2d']['y2']/float(image_height))
    ...
    def _get_boundingboxes_new_format(self, annotations_for_picture_id, image_width, image_height):
        boxid, xmin, xmax, ymin, ymax, label_id, label, truncated, occluded = \
            [], [], [], [], [], [], [], [], []
        if annotations_for_picture_id is None:
            return boxid, xmin, xmax, ymin, ymax, label_id, label, truncated, occluded
    ...
    def _get_tf_feature_dict(self, image_id, image_path, image_format, annotations, coches, personas, new_format=True):
        ...
        if not new_format:
            ...
            boxid, xmin, xmax, ymin, ymax, label_id, label, truncated, occluded = \
                self._get_boundingboxes_new_format(annotations, image_width, image_height)
        ...
        tmp_feat_dict['image/source_id'] = bytes_feature(image_fileid.encode('utf8'))
        ...
        tmp_feat_dict['image/format'] = bytes_feature(image_format.encode('utf8'))
        tmp_feat_dict['image/filename'] = bytes_feature(image_filename.encode('utf8'))
        ...
        for label in label_id:
           #print("Etiqueta")
           #print(label)
           #print(int64_feature(label))
           #print(type(bytes_feature(label)))
           if label == 3:
              coches +=1
           elif label == 1:
              personas +=1
        ...
        tmp_feat_dict['image/object/class/text'] = bytes_feature(label_bytes)
        #tmp_feat_dict['image/object/class/label/name'] = bytes_feature(label_bytes)
        #print(tmp_feat_dict)        
        return tmp_feat_dict, coches, personas
    ...
    def _get_tf_feature(self, image_id, image_path, image_format, annotations, coches, personas, new_format=True):
        """
        ...
        :return:
        """
        feature_dict, coches, personas = self._get_tf_feature_dict(
            image_id, image_path, image_format, annotations, coches, personas, new_format)
        
        return tf.train.Features(feature=feature_dict), coches, personas
    ...
    def get_output_file_name_template(
        ...
        if classes is not None:
            extra_parts += 'classes_{0}_'.format(classes.replace(',','_'))
        return os.path.join(
            output_path,
        ...
        with tf.Session() as sess:
            ...
            for file_counter, f in enumerate(image_files):
                ...
                if daytime_type is not None and \
                    ...
                    continue
                """
                print(picture_id_annotations['labels'])
                print(classes)
                print(classes_split)
                                    for pic_id in picture_id_annotations['labels']:
                        print(pic_id.get('category', None))
                        print(type(pic_id.get('category', None)))
                        print(pic_id.get('category', None) in classes_split)"""
                ...
                """print(picture_id_annotations)
                print("\n\n\n")
                print(classes)
                print(m.group(2))
                
                print("\n\n\n")"""
                feature, coches, personas = self._get_tf_feature(
                    picture_id, os.path.join(full_images_path, f),
                    m.group(2), picture_id_annotations, coches, personas, new_format)
